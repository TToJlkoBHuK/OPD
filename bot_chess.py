...
# --- КОНФИГУРАЦИЯ ---
API_TOKEN = ''
ADMIN_IDS = [1881684121, 5312321185]
...
USERS_FILE = "users.txt"            # Статусы пользователей
NICKNAMES_FILE = "nicknames.txt"    # Ники пользователей
BROADCAST_TEMPLATE_FILE = "broadcast_template.txt" # Шаблон рассылки
...
current_group_index: Dict[int, int] = {} # {user_id: last_sent_group_index}
user_bans: Dict[int, float] = {}         # {user_id: ban_end_timestamp}
# users_status: Dict[int, str] = {}      # {user_id: status_string}
...
media_groups: Dict[int, List[types.Message]] = {} # Временное хранение медиа для группировки
last_media_time: Dict[int, float] = {}      # Время последнего получения медиа
...
def get_user_keyboard() -> InlineKeyboardMarkup:
    ...
    keyboard.add(
        ...
        InlineKeyboardButton("Проверить медиа 🧐", callback_data="check_media")
    )
    keyboard.add(
...
def get_users_list_keyboard() -> InlineKeyboardMarkup:
    ...
    sorted_user_ids = sorted(users_status.keys()) 

    for user_id in sorted_user_ids:
        if user_id in ADMIN_IDS: continue
...
async def reset_groups(message: types.Message):
    ...
    # Сбрасываем прогресс у всех пользователей
    sent_groups.clear()
...
async def remove_ban(callback_query: types.CallbackQuery):
    ...
    try:
        ...
        await callback_query.answer("Бан снят.")
         ...
         await callback_query.answer("Бан снят.", show_alert=True)
...
async def process_accumulated_media(user_id: int):
    ...
    user_folder = os.path.join(MEDIA_FOLDER, str(user_id))
    username = messages_to_process[0].from_user.username or f"ID: {user_id}"
    ...
    for message in messages_to_process:
        file_info = None
        file_ext = ".dat"
        file_name_base = f"{time.time_ns()}_{file_counter}"
